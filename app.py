...
st.sidebar.markdown("---")
st.sidebar.subheader("⚙️ Canlı Veri Kaynağı")
...
# --- 5. ÜST SEKMELER (TABS) ---
tab_sim, tab_gecmis = st.tabs(["📊 Yeni Kampanya Simülatörü", "📁 Geçmiş Kampanya & Plan Analizi"])

# ==========================================
# SEKME 1: YENİ KAMPANYA SİMÜLATÖRÜ
# ==========================================
with tab_sim:
    if df_gost is not None and not df_gost.empty:
        ...
        with col1:
            ...
            secilen_il = st.selectbox("📍 İl:", il_listesi, key="sim_il")
        
        with col2:
            uniteler = sorted(list(set(df_gost[df_gost['İl'] == secilen_il]['Ünite'].tolist())))
            secilen_unite = st.selectbox("🎯 Ünite:", uniteler, key="sim_unite")
...
# ==========================================
# SEKME 2: GEÇMİŞ KAMPANYA & RAPOR ANALİZİ
# ==========================================
with tab_gecmis:
    st.subheader("📂 Geçmiş Kampanya Dosyası Yükle & Analiz Et")
    uploaded_file = st.file_uploader("Önceden hazırlanmış Excel veya CSV plan dosyasını seçin:", type=["xlsx", "xls", "csv"])
    ...
    if uploaded_file is not None:
        try:
            if uploaded_file.name.endswith(".csv"):
                ...
                df_past = pd.read_excel(uploaded_file)

            st.success(f"✅ `{uploaded_file.name}` başarıyla yüklendi! ({len(df_past)} satır)")
            st.dataframe(df_past, use_container_width=True)

            # Geçmiş Kampanya KPI Hesaplamaları
            gos_col = next((c for c in df_past.columns if "gösterim" in c.lower() or "impression" in c.lower()), None)
            grp_col = next((c for c in df_past.columns if "grp" in c.lower()), None)
            il_col = next((c for c in df_past.columns if "il" in c.lower() or "şehir" in c.lower() or "city" in c.lower()), None)

            st.markdown("---")
            st.markdown("### 📌 Kampanya Performans Özeti")
            pkpi1, pkpi2, pkpi3 = st.columns(3)
            
            if gos_col:
                pkpi1.metric("📊 Toplam Gösterim", f"{df_past[gos_col].sum():,}")
            if grp_col:
                pkpi2.metric("🇹🇷 Toplam GRP", f"{df_past[grp_col].sum():.2f}")
            if il_col:
                pkpi3.metric("📍 Kapsanan İl Sayısı", f"{df_past[il_col].nunique()} İl")

            # Looker Studio Haritası (Geçmiş kampanya için de gösterim)
            if looker_url:
                st.markdown("### 🗺️ Kampanya Konum Haritası (Looker Studio)")
                st.components.v1.html(
                    f'<iframe src="{looker_url}" width="100%" height="520" frameborder="0" style="border:0; border-radius: 8px;" allowfullscreen></iframe>',
                    height=540
                )
        ...
        st.info("💡 Müşteriye sunduğun veya arşivindeki herhangi bir Excel/CSV medya planını buraya yükleyerek özet metrikleri ve harita konumunu anında görüntüleyebilirsin.")
